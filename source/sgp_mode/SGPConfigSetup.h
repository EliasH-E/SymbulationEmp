...
EMP_EXTEND_CONFIG(SymConfigSGP, SymConfigBase,
  ...
  VALUE(HOST_MIN_CYCLES_BEFORE_REPRO, size_t, 100, "Number of CPU cycles organisms must wait between reproductions"),
  VALUE(SYM_MIN_CYCLES_BEFORE_REPRO, size_t, 10, "Number of CPU cycles organisms must wait between reproductions"),
  ...
  VALUE(TASK_PROFILE_COMPATIBILITY_MODE, std::string, "task-any-match", "How is compatibility determined for task profiles? always, task-any-match, task-perfect-match"),
  VALUE(HORIZONTAL_TRANSMISSION_COMPATIBILITY_MODE, std::string, "task-profile-strictly-stronger-match", "How is compatibility determined for horizontal transmission? Relies on TASK_PROFILE_COMPATIBILITY_MODE being set appropriately. Options are: always, task-profile-compatible, task-profile-strictly-stronger-match, task-profile-stronger-or-equal-match"),
  ...
  VALUE(STRESS_TYPE, std::string, "parasite", "What kind of stress symbionts should be incorporated in stressful environments? (Options: 'mutualist', 'parasite', 'neutral')"),
  VALUE(STRESS_FREQUENCY, size_t, 25, "How often should stress events occur (in updates)?"),
  ...
  VALUE(BASE_DEATH_CHANCE, double, 0.0, "What death chance does a host have in the absence of symbionts?"),
  ...
  VALUE(HEALTH_TYPE, std::string, "parasite", "What kind of health symbionts are used?"),
  ...
  VALUE(MUTUALIST_CYCLE_DONATE_MULTIPLIER, double, 4, "Multiply cycles donated by symbiont by this value"),
  ...
  VALUE(HEALTH_INTERACTION_CHANCE, double, 1.0, "Probability of host-symbiont health interaction each update"),

  GROUP(NUTRIENT, "Nutrient interaction settings"),
  VALUE(ENABLE_NUTRIENT, bool, false, "Nutrient interactions enabled?"),
  VALUE(NUTRIENT_TYPE, std::string, "parasite", "What kind of nutrient symbionts are used?"),
  VALUE(NUTRIENT_DONATE_PROP, double, 0.75, "When symbiont donates resources to a host, what proportion is donated?"),
  VALUE(NUTRIENT_STEAL_PROP, double, 0.75, "When symbiont steals resources from a host, what proportion is stolen?"),
  ...
  VALUE(SYM_ONLY_FIRST_TASK_CREDIT, bool, true, "Only give sym credit for one task (whatever they do first)?"),

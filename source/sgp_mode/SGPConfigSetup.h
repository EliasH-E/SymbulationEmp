...
enum TrackParentTaskType {CURRENTLONLY = 0,PARENTONLY,CURRENTORPARENT}
enum PreferentialOustingType {OFF=0, EQUALORBETTER, STRICTLYBETTER}
...
EMP_EXTEND_CONFIG(SymConfigSGP, SymConfigBase,
  ...
  VALUE(TRACK_PARENT_TASKS, int, CURRENTLONLY, "Should parental task completion data be used for reproductive task matching (instead of the individual's task completion data?) (0 to use only current tasks, 1 to use only the parental tasks, 2 to use an OR of the parent and current tasks)"),
  ...
  VALUE(TASK_MATCH_FOR_SYMBIOTIC_BEHAVIOR, bool, 0, "Is a task match between host and symbiont required for symbiotic behavior (e.g. bestowal of mutualist death change, in stress mode) to occur? (0 for no, 1 for yes)"),
